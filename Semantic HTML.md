...
**HTML** signifie « _HyperText Markup Language_ » = « langage de balises pour l'hypertexte ». 
...
**Le fil de fer de HTLM doit être respecté :**
...
En HTML, par exemple, l'élément HTML `[<h1>](https://developer.mozilla.org/fr/docs/Web/HTML/Reference/Elements/Heading_Elements)` est un élément sémantique qui donne au texte qu'il contient le rôle (ou le sens) de « titre de premier niveau de votre page ».
...
Par défaut, la plupart des [feuilles de style de l'agent utilisateur](https://developer.mozilla.org/fr/docs/Web/CSS/Guides/Cascade/Introduction#feuilles_de_style_de_lagent_utilisateur) des navigateurs appliqueront à un `[<h1>](https://developer.mozilla.org/fr/docs/Web/HTML/Reference/Elements/Heading_Elements)` une grande taille de police pour le faire _ressembler_ à un titre (bien que vous puissiez le styliser pour qu'il ressemble à tout ce que vous voulez).
...
Le code HTML doit être écrit pour représenter les _données_ qui seront affichées, et non en fonction de leur présentation par défaut. La présentation (c'est-à-dire l'apparence) relève uniquement de la responsabilité de la [CSS](https://developer.mozilla.org/fr/docs/Web/CSS).

Voici quelques avantages à écrire un balisage sémantique :

- Les moteurs de recherche considéreront son contenu comme des mots-clés importants pour influencer le classement de la page dans les résultats de recherche (voir [SEO](https://developer.mozilla.org/fr/docs/Glossary/SEO))
...
Lorsque vous choisissez quel balisage utiliser, demandez-vous : « Quel(s) élément(s) décrivent/représentent le mieux les données que je vais afficher ? » Par exemple : s'agit-il d'une liste de données ? ; ordonnée, non ordonnée ? ; d'un article
